...
pygame.display.set_caption("KMean_DataVisualization")
...
while running:
    ...
    screen.blit(text_algorithm , (870, 450))

    # reset button
    pygame.draw.rect(screen, BLACK, (850, 550 ,150, 50))
    screen.blit(text_reset, (890,550))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if 850 < mouse_x < 1000 and 150 < mouse_y < 200:
                ...
                for p in points: # xét lần lượt các điểm dư liệu
                    distances_to_cluster = [] # lưu lại khoảng cách từ các điểm đến các điểm trong cluster
                    for c in clusters:
                        dis = distance(p,c)
                        distances_to_cluster.append(dis)

                    min_distance = min(distances_to_cluster) # khoảng cách nhỏ nhất từ 1 điểm p đến các điểm trong cluster
                    label = distances_to_cluster.index(min_distance) # tìm nhãn của phần tử có khoảng cách nhỏ nhất
                    labels.append(label)
